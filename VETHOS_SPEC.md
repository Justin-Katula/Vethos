...
- **Stockage** : objectifs, stats et historique de blocage sont séparés dans `vethos_objectives.json`, `vethos_stats.json` et `vethos_blocking_history.json`; les anciens états sont migrés au démarrage.
...
- **Blocage** : règles de session 4h / 6h / 2 jours au démarrage et en session active, notifications natives Electron, hosts/firewall conservés, AppLocker ajouté en audit/enforcement selon le mode strict, fallback process-kill si AppLocker est indisponible. Le runtime de blocage vit dans `VethosBlockingService` (service Windows SYSTEM), l'UI relaie via named pipe.
...
Architecture livrée : le blocage ne s'exécute plus dans le process UI. Le main Electron expose les IPC renderer habituels, mais relaie les commandes vers `VethosBlockingService` via `\\.\pipe\VethosServiceBridge`. Le service démarre au boot Windows en compte SYSTEM, auto-redémarre via `node-windows`, et exécute `out/service/index.js` avec le binaire Electron en mode Node (`ELECTRON_RUN_AS_NODE=1`).

Cycle de vie :

- Installation/réparation : `Vethos.exe --install-service`, lancé directement ou via le bouton « Réparer » de l'UI. L'élévation UAC est ponctuelle via `sudo-prompt`.
- Désinstallation : `Vethos.exe --uninstall-service`.
- Données : les fichiers de blocage sont migrés vers `C:\ProgramData\Vethos`; le service durcit ce dossier par ACL (SYSTEM + Administrateurs en écriture, utilisateurs en lecture).
...
- Vérifier l'état : `Get-Service VethosBlockingService`.
- Vérifier le compte : `sc.exe qc VethosBlockingService` doit afficher `LocalSystem`.
- Vérifier le pipe : le fichier spécial `\\.\pipe\VethosServiceBridge` doit répondre quand le service tourne.
- Réparer : relancer Vethos puis utiliser « Réparer », ou exécuter `Vethos.exe --install-service`.
...
| 2   | Système de blocage (hosts / process / firewall + détection dérive) | ✅ Livré (v0.2.0-blocking)                        | [2026-05-04-vethos-blocking-design.md](docs/superpowers/specs/2026-05-04-vethos-blocking-design.md)     |
| 3   | Interface principale (cercle 24h, calendrier, tableau couleurs)    | ✅ Livré (v0.3.0-interface)                       | [2026-05-05-vethos-interface-design.md](docs/superpowers/specs/2026-05-05-vethos-interface-design.md)   |
| 4   | Système de niveaux + distribution du temps libre                   | ✅ Refondu (Tâches, multiplicateurs, dégradation) | [2026-05-05-vethos-levels-design.md](docs/superpowers/specs/2026-05-05-vethos-levels-design.md)         |
| 5   | Onboarding (emploi du temps + objectifs)                           | ✅ Refondu V2 (sans étape Apps)                   | [2026-05-05-vethos-onboarding-design.md](docs/superpowers/specs/2026-05-05-vethos-onboarding-design.md) |
| 6   | Polish + persistance complète (tous les `vethos_*.json`)            | ✅ Livré (v0.6.0-polish)                          | [2026-05-05-vethos-polish-design.md](docs/superpowers/specs/2026-05-05-vethos-polish-design.md)         |
