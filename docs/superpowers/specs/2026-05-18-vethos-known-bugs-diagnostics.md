# Vethos — 3 bugs connus : diagnostic + plan de fix
...
ou UAC accepté mais le service n'apparaît pas (`Get-Service VethosBlockingService`
introuvable).
...
- Si **non** : `sudo-prompt` échoue. Inspecter
  `%APPDATA%\Vethos\logs\main.log`, chercher `elevated-install` ou
  `relaunchElevated`.
- Si **oui** : passer à 2.

**2. Une seconde instance Vethos.exe (élevée) démarre-t-elle brièvement ?**
- Gestionnaire des tâches → cherche une 2ᵉ `Vethos.exe` qui apparaît puis
  disparaît.
...
- Chemin :
  `%LOCALAPPDATA%\Programs\Vethos\resources\app.asar.unpacked\out\service\daemon\`
- Doit contenir `vethosblockingservice.exe` et `vethosblockingservice.xml`.
...
- PowerShell : `Get-Service VethosBlockingService`
- Si « Service not found » → l'étape 3 a échoué.
- Si trouvé mais Status `Stopped` → install OK, démarrage qui échoue. Voir
  Event Viewer → Logs Windows → Application → filtrer sur VethosBlockingService.

**5. Lancer l'install manuellement** (bypass de l'UI) :

```powershell
& "$env:LOCALAPPDATA\Programs\Vethos\Vethos.exe" --install-service
```
...
- Ré-installer Vethos depuis l'installeur le plus récent.
...
- Vérifier antivirus / Defender → exception pour `Vethos.exe`.
...
- Vérifier que le chemin de `Vethos.exe` dans le XML est **absolu**.
- Tester manuellement : `Start-Service VethosBlockingService`. Si code 1066,
...
- Exposé via `vethos.app.discoverInstalledApps()` (IPC).
...
- Stockage dans `vethos_discovered_sites.json`.

### Diagnostic

**1. Setting bien activé ?**
- Inspecter `%APPDATA%\Vethos\vethos_settings.json` →
...
- **Cause la plus probable.** Tester : fermer complètement le navigateur,
  attendre 6 min, vérifier `vethos_discovered_sites.json`.
...
async function readHistorySafely(historyPath: string): Promise<Buffer | null> {
  const tmp = path.join(os.tmpdir(), `vethos-history-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`)
  try {
