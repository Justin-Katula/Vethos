# Vethos — P16 Phase 3 : vrai service Windows

**Auteur :** Obed (avec Claude)
**Date :** 2026-05-17
**Statut :** Design validé (plan d'attaque). Prêt pour la planification du Lot 1.
**Réf. :** Spec maître `docs/superpowers/specs/2026-05-15-vethos-windows-service-design.md` — §5, §7, §9, §11 Phase 3.
...
La Phase 3 fait de Vethos un **vrai service Windows** :
...
- node-windows lance par défaut le script de service via `node`. Le service Vethos
...
Objectif : prouver ou réfuter que node-windows peut installer Vethos en vrai
service Windows tournant sur le binaire Electron en mode Node.

- Ajouter `node-windows` aux dépendances.
- Point d'entrée `Vethos.exe --install-service` / `--uninstall-service`, détecté
...
- Service installé : `VethosBlockingService`, configuré pour exécuter le bundle
  `out/service/index.js` via le binaire Electron en mode Node, auto-restart activé.
- **Vérification — manuelle** (`VETHOS_INTEG`, inhérent à un service Windows) :
...
- `src/main/elevated-install.ts` : relance `Vethos.exe --install-service` via
...
Durcissement : ACL du pipe nommé et de `C:\ProgramData\Vethos`,
reconnexion / backoff affinés du client pipe, notification « service non
démarré », mise à jour de `VETHOS_SPEC.md`.
...
`vethos-service-phase3`, depuis `master` (`7376111`). La Phase 3 sera mergée dans
