...
Le bridge Nexus écoute sur `\\.\pipe\NexusServiceBridge` via `net.createServer()`.
...
protocole local Nexus, mais il ne peut pas encore prouver l'identité du client au
niveau pipe.

## Décision

Pour Phase 4.1, on durcit immédiatement `C:\ProgramData\Nexus` par ACL, car c'est
