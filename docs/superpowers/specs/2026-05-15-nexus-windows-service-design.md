# Nexus — Service Windows de blocage (V3 · Problème 16)
...
l'utilisateur ouvre le Gestionnaire des tâches et tue `Nexus.exe`, le blocage
s'arrête : le drift detector ne tourne plus, hosts et firewall ne sont plus
surveillés. Nexus n'est donc pas un « vrai » bloqueur.
...
| Communication par named pipe (pas TCP) | V3 P16 | `\\.\pipe\NexusServiceBridge`, isolé, non exposé au réseau |
...
```
┌─────────────────────────────────────────────────────────────┐
│                      NEXUS UI (utilisateur)                  │
│  renderer  ──IPC──▶  main                                     │
│                       │                                      │
│                       │  service-client (named pipe client)  │
│                       ▼                                      │
└───────────────────────┼──────────────────────────────────────┘
                        │   \\.\pipe\NexusServiceBridge
                        │   (requêtes corrélées + events push)
┌───────────────────────▼──────────────────────────────────────┐
│                  NEXUS SERVICE (SYSTEM, boot)                 │
...
└───────────────────────┼──────────────────────────────────────┘
              ...
              ┌──────────────────────────────┐
              │  C:\ProgramData\Nexus\        │
              │   nexus_blocking.json         │  profils + historique
              │   nexus_blocking_active.json  │  session en cours
              │   hosts.nexus.backup          │
              └──────────────────────────────┘
```
...
├── data-dir.ts                       # résout C:\ProgramData\Nexus
├── bridge/
...
└── install/
    ...
    └── migrate-legacy.ts             # copie nexus_blocking*.json → ProgramData
...
Pipe : `\\.\pipe\NexusServiceBridge`. Service = serveur (`net.createServer`), UI =
...
### 4.4 Propriété des données — `C:\ProgramData\Nexus\`
...
- **Possédés par le service** (`C:\ProgramData\Nexus\`) : `nexus_blocking.json`
  (profils + historique), `nexus_blocking_active.json` (session active),
  `hosts.nexus.backup`. Source de vérité unique. L'UI ne les lit/écrit **jamais**
  en direct — uniquement via `GET_STATE`.
- **Restent côté UI** (`%APPDATA%\Nexus`) : `settings`, `schedule`, `objectives`,
  `stats`, `app-usage`, etc. — tout ce qui n'est pas du blocage.
- Le service crée `C:\ProgramData\Nexus\` avec une ACL : écriture SYSTEM/Admins,
...
- L'installation se fait en relançant le binaire Nexus avec un flag :
  `Nexus.exe --install-service <userDataPath>`, via `sudo-prompt` (élévation UAC
  ponctuelle).
- `src/main/index.ts` détecte ce flag **tout au début** : si présent, il exécute la
  ...
  1. `node-windows` `Service.install()` + `start()` (service `NexusBlockingService`,
     auto-restart configuré).
  2. **Migration** : copie les `nexus_blocking*.json` et `hosts.nexus.backup`
     existants depuis `<userDataPath>` vers `C:\ProgramData\Nexus\` (le helper est
...
- `src/main/blocking/{hosts,processes,firewall,applocker,session}` → déplacé sous
  ...
  adapters injectés).
- `src/service/index.ts` : instancie `storage(C:\ProgramData\Nexus)`, le session
...
- **Service down en pleine session** : les règles hosts/firewall posées restent
  ...
  `nexus_blocking_active.json`. Le nettoyage des orphelins en cas d'arrêt
...
1. **node-windows + binaire Electron.** node-windows assume `node` ; il faut le
   faire pointer sur `Nexus.exe` avec `ELECTRON_RUN_AS_NODE=1`. Spike : installer un
...
- `service-status` : logique installé/démarré/à-réparer testée avec un contrôle de
  service mocké.
- **Intégration** (manuel, `NEXUS_INTEG=1`) : installation réelle du service
...
- **Phase 4 — Durcissement.** ACL du pipe, reconnexion/backoff, notification
  « service non démarré » (réclamée aussi par P22), nettoyage, `NEXUS_SPEC.md`.
...
2. Tuer `Nexus.exe` (UI) pendant une session active **n'arrête pas** le blocage
...
8. Les fichiers `nexus_blocking*.json` sont migrés vers `C:\ProgramData\Nexus\` ;
