# Nexus — Sous-projet 5 : Onboarding + apps déclarées
...
Persisté sous `nexus_declared_apps.json`.
...
- Logo Nexus animé (gradient ring qui pulse).
...
- `src/renderer/src/store/declared-apps.store.test.ts` (avec mock `nexus.storage`) :
  - load/saveApp/deleteApp respectent le schéma
  - update préserve `createdAt`

## 6. Démo bout-en-bout (acceptance)

1. Démarrer Nexus pour la première fois (suppr `nexus_settings.json` et `nexus_declared_apps.json`).
...
7. ✅ NEXUS_SPEC.md mis à jour, tag `v0.5.0-onboarding`
