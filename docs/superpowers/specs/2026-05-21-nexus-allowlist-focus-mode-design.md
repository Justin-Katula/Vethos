...
Quand il est actif, les listes du profil deviennent l'ensemble **autorisé**, et Nexus
...
- En liste blanche : Nexus ferme en continu les **apps à fenêtre visible** qui ne sont
...
- Pas de bascule du pare-feu Windows en default-deny global (trop risqué, casserait
  tout le système et Nexus lui-même).
...
de l'existant : le shell (déjà `explorer.exe`), les exes Nexus (UI + service /
`electron`), et idéalement les antivirus courants. Ajouts à valider à
l'implémentation ; ne jamais permettre que le mode focus ferme le shell, Nexus, ou la
sécurité.
