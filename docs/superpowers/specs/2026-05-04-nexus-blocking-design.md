# Nexus — Sous-projet 2 : Système de blocage
...
Implémenter le **moteur de blocage multi-couches** de Nexus. À la fin du sous-projet,
...
5. Au crash ou redémarrage de Windows pendant une session : Nexus retrouve l'état
...
└──────────────────────────────────────────────────────────────┘
              ...
              ┌────────────────────────┐
              │  PERSISTANCE (JSON)    │
              │  nexus_blocking.json   │  ← profiles, history
              │  nexus_blocking_active │  ← session en cours
              │     .json              │
              │  hosts.nexus.backup    │  ← copie originale du hosts
              └────────────────────────┘
```
...
├── hosts/
│   ├── parser.ts                 # parse hosts file + extract Nexus block
...
│   ├── persistence.ts            # nexus_blocking{,_active}.json IO
...
- Au PREMIER démarrage avant toute modification : copier le hosts existant vers
  `app.getPath('userData')/hosts.nexus.backup` (atomique). Ne JAMAIS écraser ce
  backup ensuite.
- Toutes les écritures du hosts par Nexus utilisent ce format avec sentinels :
  ```
  # === NEXUS BLOCKING START — DO NOT EDIT (managed by Nexus) ===
...
netsh advfirewall firewall add rule
  name="Nexus_Block_<sessionId>_<exeBasename>"
  dir=out
  action=block
...
`nexus_blocking_active.json` pour survie au crash).
...
- `idle → starting` : crée session ID, snapshot du profile, persiste `nexus_blocking_active.json` AVANT d'appliquer les couches
...
- `ending → idle` : rollback hosts → firewall → processes (ordre inverse), append
  history, clear `nexus_blocking_active.json`, emit événement renderer

### 4.5 Drift detector

`setInterval(checkDrift, 5000)`. Pour chaque couche :
- **hosts** : ré-ouvre le fichier, parse le bloc Nexus. Si le bloc manque OU son
...
- **firewall** : `netsh advfirewall firewall show rule name=Nexus_Block_*`. Si
...
- Stratégie : **manifest UAC** dans `electron-builder.yml` :
  ...
  ```
  → l'utilisateur voit une prompt UAC au lancement de Nexus. C'est honnête et clair.
...
- `hosts/parser` : parse `hosts` avec et sans bloc Nexus, multi-blocs (corruption),
...
Tests d'intégration (skippés sauf si `process.env.NEXUS_INTEG=1` pour pouvoir
...
5. Éditer manuellement le hosts file (retirer une ligne) → 5 secondes plus tard,
   Nexus la remet (toast « Couche restaurée »)
...
9. Soumettre 100+ mots → session arrêtée, hosts file restauré (bloc Nexus disparu),
   notepad re-lançable
...
6. ✅ `nexus_blocking{,_active}.json` persistés via le storage atomique du sous-projet 1
...
12. ✅ `NEXUS_SPEC.md` mis à jour : sous-projet 2 ✅
