# Nexus — Blocage piloté par le service (« Calendrier vivant », couche 3)
...
- Le service écrit le plan sur `C:\ProgramData\Nexus\nexus_plan.json` à
  chaque push.
...
**Renderer :**
- `src/preload/index.ts` : exposer `nexus.blocking.pushPlan`.
...
- **Disk write fréquent** : chaque push réécrit `nexus_plan.json` (24 h de
