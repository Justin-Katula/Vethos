# Vethos — Sous-projet 1 : Fondation
...
Vethos est une application desktop Windows de productivité/focus de pointe, à construire en 6 sous-projets indépendants :
...
Ce sous-projet livre uniquement le squelette technique : Electron + React + TypeScript démarrent, le thème sombre est posé, l'IPC et le stockage JSON fonctionnent bout en bout, l'installer Windows est buildable. **Aucune logique métier de Vethos ici.**
...
│  ├─ preload/
│  │   └─ index.ts                  # contextBridge.exposeInMainWorld('vethos', api)
│  ├─ renderer/
│  │   ├─ index.html
│  │   └─ src/
...
│  │       │   └─ fonts.css
│  │       └─ lib/
│  │           ├─ ipc.ts            # wrapper typé sur window.vethos
...
├─ .prettierrc
├─ package.json
└─ VETHOS_SPEC.md                    # vue d'ensemble, mise à jour à chaque sous-projet
```

### IPC — contrat

Un seul objet exposé au renderer via `contextBridge` : `window.vethos`.
...
- Tous les fichiers JSON vivent dans `app.getPath('userData')` (sur Windows : `%APPDATA%/vethos/`).
- Convention de nommage : `vethos_<domain>.json` (ex. `vethos_settings.json`).
- **Écriture atomique** :
...
Cycle complet :
1. Au montage de la page Paramètres, le store Zustand `settings` charge `vethos_settings.json` via IPC. Si le fichier n'existe pas, les champs sont vides.
2. User tape un nom → clique Sauvegarder
3. Renderer appelle `window.vethos.storage.write('settings', { username, savedAt: new Date().toISOString() })`
4. Preload forward vers main via IPC
5. Main valide via Zod, écrit atomiquement dans `vethos_settings.json`
...
7. `npm run build` produit `Vethos-Setup-0.1.0.exe` (NSIS) et un build portable
8. ESLint + Prettier passent sans erreur sur tout le code
9. `tsc --noEmit` passe sans erreur
10. `VETHOS_SPEC.md` existe à la racine et liste les 6 sous-projets avec ce sous-projet marqué comme livré
...
À la fin :
- Un repo Vethos initialisé (git), structuré comme décrit
