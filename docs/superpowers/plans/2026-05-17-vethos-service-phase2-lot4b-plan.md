...
Réf. spec : `docs/superpowers/specs/2026-05-15-vethos-windows-service-design.md` §5, §6, §11.
...
**Ce que Lot 4b fait :**
1. `migrate-blocking-data.ts` — copie idempotente des `vethos_blocking*.json` +
   `hosts.vethos.backup` de `%APPDATA%\Vethos` vers `C:\ProgramData\Vethos`.
...
- L'ACL de `C:\ProgramData\Vethos` et du pipe — Phase 4.
...
- **Migration avant lancement, idempotente.** La copie ne se fait que vers un
  ...
  la 1ʳᵉ migration — on ne réécrit jamais par-dessus). `hosts.vethos.staging`
...
`%APPDATA%\Vethos`. Le service les lit dans `C:\ProgramData\Vethos` (spec §4.4). Ce
 ...
 * pipe : migre les fichiers de blocage vers `C:\ProgramData\Vethos`, puis lance
...
```ts
  ...
  // pas déjà, après migration des données vers C:\ProgramData\Vethos. Placé
...
- `migrateBlockingData` copie les 4 fichiers de blocage de `%APPDATA%` vers
  `C:\ProgramData\Vethos` sans jamais écraser une cible existante.
...
branche `vethos-service-phase2` peut être mergée dans `master`.
...
- **Phase 4** — durcissement : ACL du pipe et de `C:\ProgramData\Vethos`,
  reconnexion/backoff affinés, `VETHOS_SPEC.md`.
