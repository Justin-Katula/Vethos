...
**Références.**
- Design Phase 3 : `docs/superpowers/specs/2026-05-17-nexus-windows-service-phase3-design.md`
- Plan du Lot 1 (spike) : `docs/superpowers/plans/2026-05-17-nexus-service-phase3-lot1-plan.md`
- Spec maître : `docs/superpowers/specs/2026-05-15-nexus-windows-service-design.md` (§5, §7, §9, §11)

**Branche.** `nexus-service-phase3` (depuis `master` `7376111`). Phase 4 : branche
...
> Lot 1 réussit** : que node-windows installe bien Nexus comme service Windows
...
1. **`elevated-install.ts`** — relance Nexus en mode install/désinstall via UAC.
   ...
   function relaunchElevated(flag: '--install-service' | '--uninstall-service'): Promise<boolean> {
     return new Promise((resolve) => {
       sudoExec(`"${process.execPath}" ${flag}`, { name: 'Nexus' }, (err) => {
         if (err) {
...
2. **Migration intégrée à l'install.** Dans `service-install.ts`, `installService()`
   ...
   `serviceDataDir` de `@service/data-dir`. La migration vers `C:\ProgramData\Nexus`
...
**Vérification manuelle** (Claude la demandera) : `Nexus.exe --install-service`
depuis un contexte non élevé → UAC → service installé, et `nexus_blocking*.json`
copiés dans `C:\ProgramData\Nexus`.
...
2. **`service-status.ts`** — `getServiceStatus(): Promise<ServiceStatus>` avec
   ...
   - (Distinguer « non installé » vs « installé mais arrêté » via `sc query
     NexusBlockingService` est un *plus* — utile pour le message du bandeau ;
...
**Fin de Phase 3.** Merger `nexus-service-phase3` dans `master`. ⚠️ Resynchroniser
...
Branche neuve `nexus-service-phase4` depuis `master`, après le merge de la Phase 3.
...
- `C:\ProgramData\Nexus` : à la création du dossier par le service, poser une ACL —
...
- Named pipe : le restreindre aux utilisateurs interactifs locaux, refuser l'accès
  ...
  le périmètre d'exposition réel du pipe `\\.\pipe\NexusServiceBridge` (par défaut
...
- Mettre à jour `NEXUS_SPEC.md` : architecture du service Windows, cycle de vie
...
`GET_LAYER_STATUS` réel ; lint + typecheck + tests verts ; `NEXUS_SPEC.md` à jour.
...
Codex implémente lot par lot, un commit par lot, sur `nexus-service-phase3` (puis
`nexus-service-phase4`). Claude vérifie chaque lot après coup (conformité au plan +
