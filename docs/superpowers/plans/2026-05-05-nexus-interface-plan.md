...
**Spec :** `docs/superpowers/specs/2026-05-05-nexus-interface-design.md`
**Date :** 2026-05-05
...
- `load()` : `nexus.storage.read('schedule')`, hydrate.
- `saveRule`, `deleteRule` (cascade), `saveEntry` (validate overlap), `deleteEntry`.
- Persistance : après chaque mutation, `nexus.storage.write('schedule', { rules, entries })`.
...
## T11. Logo Nexus dans la sidebar

`src/renderer/src/components/NexusLogo.tsx` : SVG inline (recréation du wordmark "Nexus" avec N stylisé).
...
## T15. Update `NEXUS_SPEC.md` + commit + tag
