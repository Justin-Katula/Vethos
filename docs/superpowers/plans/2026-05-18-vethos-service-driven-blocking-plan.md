...
> Reference spec : `docs/superpowers/specs/2026-05-18-vethos-service-driven-blocking-design.md`
...
**Branche suggérée :** `vethos-service-driven-blocking` depuis `master`.
...
const PLAN_FILE = join(serviceDataDir(), 'vethos_plan.json')
...
de `vethos.blocking` (suivre le pattern des autres méthodes).
...
export function usePlanPush(now: Date, rangeEndStr: string): void {
  ...
  useEffect(() => {
    ...
    void vethos.blocking.pushPlan(resolved)
...
- **Persistance disque** (`vethos_plan.json`) : doit être dans
  `C:\ProgramData\Vethos\` (`serviceDataDir()`). Les ACL sur ce dossier ont
