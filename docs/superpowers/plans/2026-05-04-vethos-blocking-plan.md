# Vethos — Sous-projet 2 : Système de blocage — Plan d'implémentation
...
**Spec source :** `docs/superpowers/specs/2026-05-04-vethos-blocking-design.md`

**Politique générale :**
- TDD obligatoire pour : parsers, state machine, locks (Vitest unitaires)
- Tests d'intégration (réel hosts/netsh/tasklist) gated par `VETHOS_INTEG=1`
...
- Sécurité hosts : backup `hosts.vethos.backup` créé une fois, jamais écrasé ; sentinels exacts ; IPv4 + IPv6 ; flush DNS après chaque write
...
| `src/main/blocking/hosts/parser.ts` | Parse le hosts file → `{ outside: string, vethosBlock: VethosBlock \| null }` |
...
| `src/preload/index.ts` (modif) | + `vethos.blocking.*` API + event subscription |
...
| `VETHOS_SPEC.md` (modif) | Sous-projet 2 = ✅ |
...
export const SENTINEL_BEGIN = '# === VETHOS BLOCKING START — DO NOT EDIT (managed by Vethos) ==='
export const SENTINEL_END = '# === VETHOS BLOCKING END ==='
```
...
describe('parseHostsFile', () => {
  ...
  it('treats double Vethos block as corruption — keeps first, drops second from outside', () => {
    ...
    expect(r.vethosBlock).not.toBeNull()
    // outside ne doit contenir aucun fragment de bloc Vethos
...
export type ParsedHosts = {
  outside: string // tout sauf les blocs Vethos, avec line endings d'origine
...
export function parseHostsFile(raw: string): ParsedHosts {
  ...
  // Retirer tout autre bloc Vethos dans `after` (corruption / doublons)
...
describe('renderVethosBlock', () => {
  ...
  it('produces an empty block when no domains', () => {
    const out = renderVethosBlock({ sessionId: 'x', startedAt: '2026-05-04T10:00:00Z', domains: [] })
    expect(out).toContain(SENTINEL_BEGIN)
    expect(out).toContain(SENTINEL_END)
  })
...
describe('parseNetshShowRules', () => {
  it('extracts rule names', () => {
    expect(parseNetshShowRules(FIXTURE)).toEqual(['Vethos_Block_abc123_chrome.exe', 'SomeOtherRule'])
  })
...
import { renderVethosBlock } from '../hosts/writer'
    ...
    start(getActive, applyFirewall) {
      ...
      timer = setInterval(async () => {
        ...
        try {
          ...
          const raw = await fsp.readFile(HOSTS_PATH, 'utf8')
          const parsed = parseHostsFile(raw)
          const expected = renderVethosBlock({
            sessionId: active.id,
            startedAt: active.startedAt,
          ...
          const blockMatches =
            parsed.vethosBlock?.entries.length ===
...
export async function registerBlockingHandlers(
  ...
  const manager = createSessionManager({
    hosts: { apply: applyVethosBlock, clear: clearVethosBlock, flushDns },
...
- [ ] **Step 14.1 :** Étendre `vethos` exposé via `contextBridge`.

```ts
// src/preload/index.ts (ajouter au handle vethos)
blocking: {
...
export const useBlockingStore = create<BlockingStore>((set, get) => ({
  ...
  async load() {
    const elevated = await vethos.blocking.isElevated()
    const initial = await vethos.blocking.getInitialState()
    set({ loaded: true, elevated, state: initial.state, active: initial.active })
    vethos.blocking.onSessionChanged((s) => set({ active: s as ActiveSession | null }))
    vethos.blocking.onLayerDrift((e) => set({ driftToast: { layer: e.layer, at: Date.now() } }))
    void get().refreshLayerStatus()
  },
  async saveProfile(draft) {
    const saved = (await vethos.blocking.saveProfile(draft)) as BlockingProfile
  ...
  async deleteProfile(id) {
    await vethos.blocking.deleteProfile(id)
    set({ state: { ...get().state, profiles: get().state.profiles.filter((p) => p.id !== id) } })
  },
  async startSession(profileId, minutes) {
    const s = (await vethos.blocking.startSession({ profileId, durationMinutes: minutes })) as ActiveSession
    set({ active: s })
    void get().refreshLayerStatus()
  },
  async requestUnlock() {
    await vethos.blocking.requestUnlock()
    // active sera mis à jour via event sessionChanged
  },
  async submitJustification(text) {
    const r = (await vethos.blocking.submitJustification(text)) as { ok: boolean; reason?: string }
    return r
  },
  async refreshLayerStatus() {
    const s = (await vethos.blocking.getLayerStatus()) as LayerStatus
    set({ layerStatus: s })
  },
}))
```
      ...
      <div className="flex h-full flex-col gap-8 px-12 pt-16 pb-12 overflow-y-auto">
        ...
        {!elevated && (
          <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-4 text-amber-300">
            Vethos n'est pas en mode administrateur. Les blocages ne peuvent pas être appliqués.
            Relance Vethos avec « Exécuter en tant qu'administrateur ».
          </div>
        )}
...
- [ ] **Step 17.2 :** Build de test : `npm run build:win`. L'installer doit demander UAC au lancement de Vethos.
...
- [ ] **Step 18.6 :** Démarrer session 10 min → vérifier dans terminal :
  ...
  ```
- [ ] **Step 18.7 :** Éditer manuellement le hosts (retirer une ligne) → 5s plus tard, Vethos la remet (toast)
...
- [ ] **Step 18.10 :** Tuer Vethos pendant une session → relancer → couches restaurées, timer reprend
- [ ] **Step 18.11 :** Mettre à jour `VETHOS_SPEC.md` : sous-projet 2 = ✅ Livré, lien vers spec
...
- [ ] Survit au crash : redémarrage Vethos en milieu de session = couches restaurées
- [ ] UI 11/10 (cohérent avec sous-projet 1)
- [ ] `requestedExecutionLevel: requireAdministrator` dans le build prod
- [ ] `VETHOS_SPEC.md` mis à jour
