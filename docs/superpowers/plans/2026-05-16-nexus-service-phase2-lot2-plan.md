...
**Note logger :** `@main/logging/setup` (electron-log/main) écrit dans `nexus.log`
...
d'`electron-log/node` au lieu de `nexus.log`. C'est cosmétique (diagnostics
...
Ce câblage garantit que les chemins `hosts.nexus.backup` / `hosts.nexus.staging`
