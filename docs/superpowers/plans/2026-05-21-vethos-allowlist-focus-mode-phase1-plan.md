...
**Spec :** `docs/superpowers/specs/2026-05-21-vethos-allowlist-focus-mode-design.md`
...
- `src/service/blocking/processes/safe-list.ts` (modif) — durcir (UI Vethos).
...
## Task 4: Durcir la safe-list (ne jamais tuer l'UI Vethos)
...
describe('isSafeListed', () => {
  ...
  it("protège l'UI Vethos pour qu'un focus ne la ferme jamais", () => {
    expect(isSafeListed('Vethos.exe')).toBe(true)
...
Expected: FAIL — `Vethos.exe`/`electron.exe` ne sont pas encore safe-listés.

- [ ] **Step 3: Add Vethos UI exes to the safe-list**

In `src/service/blocking/processes/safe-list.ts`, add to the `SYSTEM_SAFE_LIST` set (before the closing `])`):

```ts
  'searchindexer.exe',
  // UI Vethos : un mode focus (liste blanche) ne doit JAMAIS fermer l'app elle-même.
  'vethos.exe',
  'electron.exe',
])
```
...
git commit -m "feat(blocking): safe-list l'UI Vethos pour le mode focus"
```
...
> Ce test valide le cœur du comportement. À faire sur une vraie machine Windows avec le service Vethos installé et joignable.
...
4. Vérifie : Notepad reste ouvert ; la Calculatrice (et toute autre app à fenêtre non autorisée) est fermée et le reste ; l'UI Vethos et l'explorateur ne sont jamais fermés.
...
7. Bonus : pendant la session, vérifie que le fichier hosts ne contient PAS de bloc Vethos (le détecteur de dérive ne doit rien écrire en liste blanche).
