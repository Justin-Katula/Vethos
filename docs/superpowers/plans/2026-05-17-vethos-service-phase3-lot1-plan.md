...
**Goal:** Dé-risquer le point technique #1 du sous-projet P16 : prouver (ou réfuter) que node-windows peut installer Vethos comme un vrai service Windows tournant sur le binaire Electron en mode Node.
...
Réf. : design Phase 3 `docs/superpowers/specs/2026-05-17-vethos-windows-service-phase3-design.md` §3 Lot 1, et spec maître §5, §9.
...
**Hypothèse à valider :** la routine d'install tourne sous `Vethos.exe --install-service`
...
**Critère de réussite du spike :** le service `VethosBlockingService` survit à un
 ...
 * sous `Vethos.exe` (binaire Electron), et avec `ELECTRON_RUN_AS_NODE=1` dans
...
/** Désinstalle `VethosBlockingService`. Idempotent côté node-windows. */
...
Quand `Vethos.exe` est lancé avec `--install-service` (resp. `--uninstall-service`),
...
- [ ] **Step 4: Encapsuler le démarrage normal dans `startVethosApp()`**
...
hors mode install. Envelopper tout ce bloc dans une fonction `startVethosApp()`.
...
(La fermeture `}` referme `startVethosApp`. `startVethosApp()` est appelée dans la
...
l'indentation du bloc `startVethosApp`, lancer `npm run format` puis re-vérifier.
...
2. **Installer :** lancer `Vethos.exe --install-service` depuis l'app packagée.
   Vérifier : aucune fenêtre ne s'ouvre, le process quitte (code 0), et
   `services.msc` (ou `sc query VethosBlockingService`) montre le service installé.
3. **Démarrage & SYSTEM :** vérifier que le service est `Running` et que sa
   ...
   Vérifier `C:\ProgramData\Vethos\logs\vethos-service.log` : le service a logué
   son démarrage.
4. **Blocage :** ouvrir l'UI Vethos, démarrer une session de blocage, confirmer
...
5. **Survie au reboot :** redémarrer Windows. Sans ouvrir l'UI, vérifier que
   `VethosBlockingService` est de nouveau `Running` (démarrage automatique au boot).
...
7. **Désinstaller :** `Vethos.exe --uninstall-service` ; vérifier que le service
