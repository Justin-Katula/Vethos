...
`docs/superpowers/specs/2026-05-15-nexus-windows-service-design.md`. Les Phases 2-4
...
CRÉÉ :
  ...
  src/service/data-dir.ts                 # résout C:\ProgramData\Nexus
...
Vérifier que le fichier `nexus-service-spike.log` a bien été écrit (chemin par
...
- Installer un service de spike `NexusBlockingServiceSpike` exécutant le heartbeat.
- `sc query NexusBlockingServiceSpike` → état `RUNNING`.
...
log.transports.file.resolvePathFn = () =>
  join(serviceDataDir(), 'logs', 'nexus-service.log')
...
`node -e "const n=require('net');const s=n.createConnection('\\\\\\\\.\\\\pipe\\\\NexusServiceBridge');s.on('connect',()=>s.write(JSON.stringify({kind:'request',id:'x',type:'PING'})+'\n'));s.on('data',d=>{console.log(d.toString());process.exit(0)})"`
...
Expected : le log du `main` (`%APPDATA%\Nexus\logs\nexus.log` ou la console dev)
...
- **Phase 2** — migration du blocage dans le service ; l'UI bascule en client pipe ;
  fichiers vers `C:\ProgramData\Nexus`. Élargit le protocole (`GET_STATE`,
...
Réf. spec : `docs/superpowers/specs/2026-05-15-nexus-windows-service-design.md` §11.
