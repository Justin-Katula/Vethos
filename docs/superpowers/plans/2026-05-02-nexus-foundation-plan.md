# Nexus — Sous-projet 1 : Fondation — Plan d'implémentation
...
**Spec source :** `docs/superpowers/specs/2026-05-02-nexus-foundation-design.md`
...
| `src/preload/index.d.ts` | Types pour `window.nexus` |
...
| `src/renderer/src/lib/ipc.ts` | Wrapper typé sur window.nexus |
| `src/renderer/src/lib/cn.ts` | Utilitaire merge classes Tailwind |
| `NEXUS_SPEC.md` | Tracker des 6 sous-projets |
...
```bash
cd C:/Users/obedi/Nexus
git init
```

Expected : `Initialized empty Git repository in C:/Users/obedi/Nexus/.git/`
...
```bash
git add .
git commit -m "chore: init Nexus repo with package.json and gitignore"
```
...
```bash
git add src/preload/
git commit -m "feat(preload): expose typed nexus API to renderer via contextBridge"
```
...
- `release/Nexus-Setup-0.1.0.exe` produit
- `release/Nexus-0.1.0.exe` (portable) produit
...
Lancer `release/Nexus-Setup-0.1.0.exe`, l'installer s'ouvre, on peut installer, l'app installée se lance correctement.
...
## Task 19 : NEXUS_SPEC.md — tracker des sous-projets

**Files :**
- Create : `NEXUS_SPEC.md`

- [ ] **Step 1 : Créer `NEXUS_SPEC.md`**
...
```bash
git add NEXUS_SPEC.md
git commit -m "docs: add NEXUS_SPEC.md tracker for the 6 sub-projects"
```
...
6. Vérifier que `%APPDATA%/nexus/nexus_settings.json` existe et contient le bon JSON
...
Expected : `release/Nexus-Setup-0.1.0.exe` produit, exécutable
...
8. ✅ `NEXUS_SPEC.md` à la racine documente les 6 sous-projets
...
- Schémas JSON pour toutes les autres entités (`nexus_tasks.json`, `nexus_objectives.json`, etc.) → Sous-projet 6
