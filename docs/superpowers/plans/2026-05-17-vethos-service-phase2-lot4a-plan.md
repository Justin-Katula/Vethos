...
Réf. spec : `docs/superpowers/specs/2026-05-15-vethos-windows-service-design.md` §4.1, §6, §7.
...
- **Lot 4b** (plan suivant) — lancement automatique du service en process détaché
  par le `main`, migration des fichiers de blocage vers `C:\ProgramData\Vethos`,
...
- Migrer les fichiers vers `C:\ProgramData\Vethos`.
...
- **`stats` reste écrit côté UI.** Le service émet `SESSION_ENDED { entry, session }` ;
  le relais déclenche `notifySessionEnd` et met `vethos_stats.json` à jour. Pour la
...
`vethos_blocking*.json` + `hosts.vethos.backup` de `%APPDATA%` vers
`C:\ProgramData\Vethos` avant le lancement ; `asarUnpack` du bundle service dans
