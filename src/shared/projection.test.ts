...
describe('l’heure écrite en français', () => {
  ...
  it('écrit la décimale avec un point', () => {
    // Le séparateur décimal de la langue de l'application.
...
describe('les paliers d’un objectif', () => {
  ...
  it('change exactement aux bornes annoncées', () => {
    expect(titre(39.9)).toBe('Getting started')
    expect(titre(40)).toBe('Solid foundations')
    expect(titre(99.9)).toBe('Solid foundations')
    expect(titre(100)).toBe('Fluency')
    expect(titre(249.9)).toBe('Fluency')
    expect(titre(250)).toBe('Proven expertise')
    expect(titre(499.9)).toBe('Proven expertise')
    expect(titre(500)).toBe('High mastery')
  })
...
describe('ce qu’une ancre devient sur un an', () => {
  it('compte les livres d’une ancre de lecture', () => {
    expect(anchorInsight('Lecture du soir', 80)).toContain('10 whole books')
  })

  it('accorde le singulier', () => {
    const texte = anchorInsight('Lecture', 8)
    expect(texte).toContain('1 whole book read')
    expect(texte).not.toContain('books')
  })
  ...
  it('ne descend jamais à zéro livre', () => {
    ...
    expect(anchorInsight('Lecture', 1)).toContain('1 whole book')
  })

  it('reconnaît le sport sous ses différents noms', () => {
    for (const nom of ['Course à pied', 'Muscu', 'Yoga du matin', 'GYM', 'Morning run']) {
      expect(anchorInsight(nom, 100), nom).toContain('physical and cardiovascular')
    }
  })
  ...
  it('reconnaît la méditation, accents compris', () => {
    ...
    expect(anchorInsight('Méditation', 50)).toContain('deep calm')
    expect(anchorInsight('Méditation du matin', 50)).toContain('deep calm')
  })

  it('reconnaît une ancre accentuée de sport', () => {
    expect(anchorInsight('Étirements & gym', 40)).toContain('physical and cardiovascular')
  })
  ...
  it('retombe sur une phrase chiffrée quand le nom ne dit rien', () => {
    ...
    expect(texte).toContain('26 hours')
    expect(texte).toContain('immovable ritual')
  })

  it('ignore la casse et les accents du nom', () => {
    expect(anchorInsight('SPORT', 10)).toContain('physical and cardiovascular')
  })
})
