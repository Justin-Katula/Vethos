...
const task = (over: Partial<TaskItem> = {}): TaskItem => ({
  ...
  plan: 'Faire les exercices de maths ce soir.',
  deadline: '2026-08-20',
  importance: 5,
  category: 'maths',
  workKind: 'routine',
...
describe('B.5 — découpage automatique', () => {
  ...
  it('durée trop courte pour deux parts valides → aucun découpage, jamais deux morceaux sous le seuil', () => {
    // 40 min avec un plafond de 30 : il « faudrait » deux parts, mais deux
    // parts de 20 violeraient le seuil de 25. On ne découpe donc pas — le
    // moteur étalera la tâche sur plusieurs jours par ses propres règles.
    expect(autoSplit({ totalMinutes: 40, maxPerDayMinutes: 30, minPartMinutes: 25 })).toEqual([])
  })

  it('quel que soit le plafond du jour, aucune part ne descend jamais sous le seuil', () => {
    // Balayage : le garde-fou de B.5 doit tenir sur toute la plage, pas
    // seulement sur les cas ronds.
    for (let total = 25; total <= 400; total += 5) {
      for (let cap = 10; cap <= 120; cap += 10) {
        const parts = autoSplit({ totalMinutes: total, maxPerDayMinutes: cap, minPartMinutes: 25 })
        expect(parts.every((p) => p.minutes >= 25)).toBe(true)
        // Rien n'est jamais perdu ni inventé en chemin.
        if (parts.length > 0) {
