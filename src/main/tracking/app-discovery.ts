 ...
 * calculée une seule fois sur l'entrée retenue, pas sur chaque doublon écarté.
 */
...
export async function discoverInstalledApps(): Promise<DiscoveredApp[]> {
  ...
  const apps = await classerApplications(await attachAppIcons(mergedApps))
...
async function lireCacheIA(): Promise<AiCache> {
  try {
    const parsed: unknown = JSON.parse(await readFile(cheminCacheIA(), 'utf8'))
  ...
  return [...finalApps.values()]
    .map(({ name, exeName, exePath, publisher, source, packageId, hasExecutablePath, iconSourcePaths }) => {
      const app: DiscoveredApp = {
        ...
        exePath,
        publisher,
        // Provisoire : `classerApplications` la recalcule sur l'entrée retenue,
        // puis l'IA reprend le reliquat. Voir la fin de discoverInstalledApps.
        category: 'others',
        source,
        hasExecutablePath,
      }
