...
describe('session-diagnostics', () => {
  it('detects invalid durations', () => {
    expect(runSessionDiagnostics({ ...sessionPlanFixture(), plannedDurationMinutes: 0 }).issues[0]!.id).toBe('invalid_duration_zero')
    expect(runSessionDiagnostics({ ...sessionPlanFixture(), plannedDurationMinutes: Number.NaN }).issues[0]!.id).toBe('invalid_duration_nan')
  })
