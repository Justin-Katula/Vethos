...
function createBrowserStub(): NexusApi {
  ...
  return {
    ...
    planning: {
      ...
      stopBlock: async () => ({ ok: true as const, step: 'stopped' as const, options: [], minutes: 0, source: { kind: 'task' as const, refId: '', blockId: '' } }),
      trust: async () => null,
      waiveStop: async () => null,
      promise: async () => null,
      emergency: async () => ({ ok: true as const }),
      breather: async () => ({ ok: true as const }),
      resume: async () => null,
