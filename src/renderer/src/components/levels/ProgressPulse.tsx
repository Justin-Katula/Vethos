 ...
 *
 * Trois rôles :
 * 1. Pulse flottant "+X min objectifs" quand `lastProgressEvent` vient d'être émis
 ...
 */
export function ProgressPulse(): JSX.Element {
  const event = useLevelsStore((s) => s.lastProgressEvent)
  const consume = useLevelsStore((s) => s.consumeProgressEvent)
  ...
  // Auto-consume du progress event après 2.4s (durée animation)
  useEffect(() => {
    if (!event) return
  ...
  return (
    <>
      ...
      <ObjectiveProgressPulse event={event} />
    </>
  )
}

function ObjectiveProgressPulse({
  event,
}: {
  event: ReturnType<typeof useLevelsStore.getState>['lastProgressEvent']
}): JSX.Element {
  return (
    <AnimatePresence>
      {event && event.objectiveDeltas.length > 0 && (
        ...
        >
          <div className="flex items-center gap-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/15 px-4 py-2 text-sm font-semibold text-emerald-200 shadow-elevated backdrop-blur-md ring-1 ring-emerald-500/20">
            <motion.span
  ...
  return (
    <AnimatePresence>
      {banner && (
        ...
        >
          <div className="flex items-center gap-2 rounded-2xl border border-accent/30 bg-accent/15 px-4 py-2 text-sm font-semibold text-accent shadow-elevated backdrop-blur-md ring-1 ring-accent/20">
