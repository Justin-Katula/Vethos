/**
 * Fond animé « pluie » — éléments qui tombent en continu via des radial-gradient
 * animés avec Framer Motion. Effet subtil, ambient, en arrière-plan de l'app.
 *
 * Adapté depuis un composant FallingPattern public, replié sur les tokens CSS
 * de Vethos (--accent pour les gouttes, --bg-base pour le fond) au lieu des
 * tokens génériques --primary/--background.
 *
 * À placer en couche absolue derrière le contenu (z-0) ; un overlay flou
 * atténue le motif pour préserver la lisibilité du contenu (z-10).
 */
...
type FallingPatternProps = React.ComponentProps<'div'> & {
  /** Couleur des éléments qui tombent (défaut : accent Vethos). */
  color?: string
  /** Couleur de fond (défaut : bg-base Vethos). */
  backgroundColor?: string
  /** Durée d'animation en secondes (défaut : 150). */
  duration?: number
  /** Intensité du flou de l'overlay (défaut : '1em'). */
  blurIntensity?: string
  /** Densité de la grille ponctuelle (défaut : 1). */
  density?: number
}
...
export function FallingPattern({
  ...
  // Génère la composition de radial-gradient qui forment les colonnes qui tombent.
  ...
  return (
    // `rest` porte notamment le masque radial qui efface le motif au centre :
    // sans lui, le fond animé passe devant le contenu au lieu de l'entourer.
    <div className={cn('relative h-full w-full p-1', className)} {...rest}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
