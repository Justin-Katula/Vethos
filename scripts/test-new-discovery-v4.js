...
async function run() {
  ...
  for (const app of sortedList) {
    if (app.name === 'Git' || app.name.includes('Detroit') || app.name.includes('Lana') || app.name.includes('Exile') || app.name.includes('Sky:')) {
      console.log(`${app.name} -> ${app.exePath || 'FAILED'}`);
    }
  }
  
  fs.writeFileSync('new-scan-results-v4.json', JSON.stringify(sortedList, null, 2), 'utf-8');
}
