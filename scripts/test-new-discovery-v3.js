...
async function run() {
  ...
  for (const item of registryItems) {
    ...
    // 1. Try to extract exe from DisplayIcon (if not steam game, or if steam game has no installer in DisplayIcon)
    if (!exePath) {
      const p = extractExePath(item.DisplayIcon || '');
      if (p && isUserFacingExe(p)) {
        // If it's a Steam game, verify it's not steam.exe
    ...
    // 2. Try to extract exe from UninstallString (only if NOT a Steam game, to avoid getting steam.exe)
    ...
    // 4. Try scanning InstallLocation (very useful for Steam games and portable installations)
  ...
  // Print Steam game resolutions
  console.log('\n--- STEAM GAME RESOLUTIONS ---');
  for (const app of sortedList) {
    if (app.publisher === 'Quantic Dream' || app.publisher === 'thatgamecompany' || app.name.includes('Lana') || app.name.includes('Exile') || app.name.includes('Cyberpunk') || app.name.includes('Detroit')) {
      console.log(`${app.name} -> ${app.exePath || 'FAILED'}`);
    }
  }
  
  fs.writeFileSync('new-scan-results-v3.json', JSON.stringify(sortedList, null, 2), 'utf-8');
}
