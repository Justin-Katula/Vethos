...
async function run() {
  ...
  // We merge all candidates. Registry is our primary source of truth for desktop apps, AppX for store apps.
  // We keep a registry entry even if it doesn't have an exe path.
  ...
  const APPX_SYSTEM_NAMES = new Set([
    ...
    'microsoft.sechealthui', 'microsoft.edge.gameassist', 'microsoft.gamingservices',
  ...
  for (const item of registryItems) {
    ...
    if (!exePath && item.InstallLocation) {
      ...
      if (loc && fs.existsSync(loc)) {
        ...
        if (exes.length > 0) {
          // Find closest match or shortest
  ...
  // 4. Add shortcuts that were not matched (e.g. portable apps or things with shortcut only)
  // But wait! We should filter out shortcuts that are system tools or uninstallers
  for (const [canonical, target] of shortcutMap.entries()) {
    // Check if already added (under a slightly different name maybe)
    let exists = false;
    for (const app of finalApps.values()) {
      if (app.exePath && app.exePath.toLowerCase() === target.toLowerCase()) {
        exists = true;
        break;
      }
    }
    if (!exists) {
      // Find shortcut details to get its original name
      const originalShortcut = shortcuts.find(s => canonicalNameKey(s.Name || '') === canonical);
      if (originalShortcut) {
        addCandidate(originalShortcut.Name, target, '', 'shortcut');
      }
    }
  }
  ...
  fs.writeFileSync('new-scan-results.json', JSON.stringify(sortedList, null, 2), 'utf-8');
  console.log('Saved results to: new-scan-results.json');
}
