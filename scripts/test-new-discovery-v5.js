...
async function run() {
  ...
  function addCandidate(key, name, exePath, publisher, source, packageId = undefined) {
  ...
  for (const pkg of appxPackages) {
    ...
    if (!isSystem && name) {
      addCandidate(`appx:${family.toLowerCase()}`, name, pkg.ExecutablePath, pkg.Publisher, 'appx', family);
    }
  }
  ...
  for (const item of registryItems) {
    ...
    addCandidate(`reg:${canonicalNameKey(name)}`, name, exePath, item.Publisher, 'registry');
  }

  const sortedList = [...finalApps.values()].sort((a, b) => a.name.localeCompare(b.name, 'fr'));
  
  console.log(`\nFinal count: ${sortedList.length} apps discovered.`);
  fs.writeFileSync('new-scan-results-v5.json', JSON.stringify(sortedList, null, 2), 'utf-8');
}
