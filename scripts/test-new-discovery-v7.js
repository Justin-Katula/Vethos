...
async function run() {
  // 1. Read Get-StartApps to map PackageFamilyName to clean localized names
  console.log('Querying Get-StartApps...');
  const startAppsScript = `Get-StartApps | ConvertTo-Json`;
  const { stdout: startAppsStdout } = await execFilePromise(
    'powershell.exe',
    ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', startAppsScript],
    { maxBuffer: 15 * 1024 * 1024 }
  );
  const startApps = JSON.parse(startAppsStdout.trim() || '[]');
  
  const startAppsMap = new Map(); // maps PackageFamilyName (lowercase) -> Name
  for (const app of startApps) {
    const appid = app.AppID || '';
    const name = app.Name || '';
    if (appid.includes('!') && name) {
      const pkgFamily = appid.split('!')[0].toLowerCase();
      // Only keep the shortest name for each package family if multiple exist
      if (!startAppsMap.has(pkgFamily) || name.length < startAppsMap.get(pkgFamily).length) {
        startAppsMap.set(pkgFamily, name);
      }
    }
  }

  // 2. Read start menu shortcuts for path resolution
  ...
  // 3. Read AppX Packages
  console.log('Reading AppX packages...');
  const appxScript = `
    $OutputEncoding = [Console]::OutputEncoding = [System.Text.Encoding]::UTF8
  ...
  // 4. Read Registry Uninstall Entries
  console.log('Reading registry entries...');
  ...
  // AppX system filters
  // NOTE: removed 'microsoft.xbox' from system prefixes so Xbox Live is allowed.
  ...
  for (const pkg of appxPackages) {
    ...
    // Resolve clean name using startAppsMap
    let name = pkg.DisplayName || '';
    if (startAppsMap.has(lowerFamily)) {
      name = startAppsMap.get(lowerFamily);
    }
  ...
  for (const item of registryItems) {
    ...
    const uninstStr = item.UninstallString || '';
    const isSteamGame = uninstStr.includes('steam://uninstall/') || uninstStr.includes('steam.exe');

    let exePath = '';

    // 1. Try to extract exe from DisplayIcon
    if (!exePath) {
      const p = extractExePath(item.DisplayIcon || '');
    ...
    // 2. Try to extract exe from UninstallString
    ...
    // 4. Try scanning InstallLocation
  ...
  fs.writeFileSync('new-scan-results-v7.json', JSON.stringify(sortedList, null, 2), 'utf-8');
}
